...
\\ \\
Now let us look at the exact symmetries of the full, perturbed Hamiltonian $ H = H_0 + H_1$, without doing perturbation theory at all. Since $[H,L_z] = 0$ the exact eigenstates of $H$ can be chosen to be eigenstates of $L_z$ as well.
...
\[H_{\rm FS} = H_{\mathrm{RKE}} + H_{\rm D} + H_{\mathrm{SO}}.\]
...
\[\langle n l j m_j | H_{\rm D} | n l j m_j\rangle = -\alpha^2 E_n \frac{1}{n}\delta_{l0},\]
...
Suppose the typical energy of the term $H_{\mathrm{i}}$ is $E_{\mathrm{i}}$. Then we have $E_a \sim {me^4}/{32 n^2 \pi^2\hbar^2\epsilon_0^2}$, $E_{\rm Z} \sim {ne\hbar B}/{2m}$ and $E_{\mathrm{B}} \sim  {2n^4\pi^2 \epsilon_0^2 \hbar^4 B^2}/{m^3 e^2}$,
and so
...
Since $[H_{\mathrm{Z}},L^2] = 0$ and $[H_{\mathrm{Z}},J_z] = 0$, off-diagonal matrix element vanishes automatically.
The energy shift is
...
\[i\hbar \frac{d^2W}{dx^2} - \left( \frac{dW}{dx} \right)^2 + p^2(x) = 0,\]
where $p^2 = 2m(E-V)$.
...
\[\left[-W'^2_0 + p^2 \right] + \hbar \left[ iW''_0 - 2W'_0W'_1 \right] + O(\hbar^2) = 0.\]
...
\[\hbar \left| \frac{dp}{dx} \right| \ll |p|^2  \Rightarrow \frac{d\lambda}{dx} \ll 2\pi,\]
...
\[\lambda \left| \frac{dV}{dx} \right| \ll \frac{|p|^2}{2m},\]
...
\[\psi(x) \approx \frac{A}{2m(V-E)^{1/4}} \exp \left( \pm \frac{1}{\hbar} \int^x dx' \sqrt{2m(V - E)}\right) .\]
...
\[u \equiv \left(\frac{2mC}{\hbar^2}\right)^{\frac{1}{3}} (x - x_0) \approx  \left(\frac{2m}{\hbar^2 C^2}\right) ^{\frac{1}{3}} (V - E).\]
...
\[\mathrm{Ai}(x) \sim \left[ \frac{(2mC\hbar)^{1/3}}{\pi \sqrt{2m(E-V}} \right]^{1/2} \cos \left[ \frac{1}{\hbar} \int_{x_0}^x dx' \mathop{\rm sgn}(C) \sqrt{2m(E-V)} + \frac{\pi}{4} \right] .\]
...
\[\mathrm{Ai}(x) \sim \frac{1}{2} \left[ \frac{(2mC\hbar)^{1/3}}{\pi \sqrt{2m(V-E)}} \right]^{1/2} \exp \left( -\frac{1}{\hbar} \int_{x_0}^x dx' \mathop{\rm sgn}(C) \sqrt{2m(V - E)} \right) .\]
...
\[\psi_1(x) \approx \frac{A}{2m(V - E)^{1/4}} \exp \left( -\frac{1}{\hbar} \int_{x}^a dx' \sqrt{2m(V-E)} \right).\]
...
\[\psi_2(x) \approx \frac{2A}{2m(E - V)^{1/4}} \cos \left( \frac{1}{\hbar} \int_{a}^x dx' \sqrt{2m(V-E)} - \frac{\pi}{4} \right).\]
...
\[\psi_1(x) \approx \frac{A'}{2m(V - E)^{1/4}} \exp \left( -\frac{1}{\hbar} \int_{b}^x dx' \sqrt{2m(V-E)} \right).\]
Matching to the Airy function across the turning point $x = b$, we have
\[\psi_2(x) \approx \frac{2A'}{2m(E - V)^{1/4}} \cos \left( \frac{1}{\hbar} \int_{b}^x dx' \sqrt{2m(V-E)} + \frac{\pi}{4} \right).\]
...
\[\int_a^b dx' \sqrt{2m(E-V)} = \left( n + \frac{1}{2} \right) \hbar \pi.\]
...
\[\oint dx \; p = \left( n + \frac{1}{2} \right) h , \quad h \equiv 2\pi \hbar ,\]
...
The standard example is a bunch of nuclei, each with position $\bm{R}_{\alpha}$ mass $M_{\alpha}$ and charge $Z_{\alpha}$, interacting with a bunch of electrons, each with position $\bm{r}_i$ , mass $m$ and charge $-e$. 
The Hamiltonian is
